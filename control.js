	...
	move2: function(creep, role, source = false)
	{
		...
		else if(Memory.rooms[room_name].path[pos.x] && Memory.rooms[room_name].path[pos.x][pos.y])	//Otherwise, follow its routine path.
		{
			...
			switch (role)
			{
				...
				case "builder":
				{
					if(creep.memory.dtrip)	//We're going to the defense.
					{
						if ((!creep.memory.return &&
							(Array.isArray(Memory.rooms[room_name].path[pos.x][pos.y].defpath) && Memory.rooms[room_name].path[pos.x][pos.y].defpath[source]
								&& Memory.rooms[room_name].path[pos.x][pos.y].defpath[source][creep.memory.need] && (tempdir = Memory.rooms[room_name].path[pos.x][pos.y].defpath[source][creep.memory.need])))
						  || (creep.memory.return && (Memory.rooms[room_name].path[pos.x][pos.y].mreturn && Memory.rooms[room_name].path[pos.x][pos.y].mreturn[Memory.rooms[room_name].defense.dshort[creep.memory.need]]
								&& (tempdir = Memory.rooms[room_name].path[pos.x][pos.y].mreturn[Memory.rooms[room_name].defense.dshort[creep.memory.need]]))
								|| (Array.isArray(Memory.rooms[room_name].path[pos.x][pos.y].dreturn) && Memory.rooms[room_name].path[pos.x][pos.y].dreturn[source]
						...
						if (Memory.rooms[room_name].path[pos.x][pos.y].flipper
							...
							||   (creep.memory.return && Memory.rooms[room_name].path[pos.x][pos.y].flipper.dreturn && Memory.rooms[room_name].path[pos.x][pos.y].flipper.dreturn[source]
								&& Memory.rooms[room_name].path[pos.x][pos.y].flipper.dreturn[source][creep.memory.need] && (!(creep.memory.dtrip = false))
								&& Memory.rooms[room_name].path[pos.x][pos.y].mreturn && Memory.rooms[room_name].path[pos.x][pos.y].mreturn[source] && (tempdir = Memory.rooms[room_name].path[pos.x][pos.y].mreturn[source])
							)))
						{
				...
				case "mtransport":
				{
					if (!Memory.creeps[creep.name].dtrip && Memory.creeps[creep.name].utrip)	//We're going to the upgrader.
					{
						...
						if (Memory.rooms[room_name].path[pos.x][pos.y].flipper
							...
							&& Memory.rooms[room_name].path[pos.x][pos.y].mine && (creep.memory.direction = Memory.rooms[room_name].path[pos.x][pos.y].mine[source])
							//If we're a builder, go to the dbuilder.
							&& ((role === 'builder' && Memory.rooms[creep.room.name].creeps.dbuilder.length > 0 && (!(creep.memory.return = false)) && (creep.memory.dtrip = true)) || true))
							))
						{
							//We get here if we have flipped any flag.
						}
					...
					else if (!Memory.creeps[creep.name].dtrip)	//We're going to the source.
					{
						...
						if (Memory.rooms[room_name].path[pos.x][pos.y].flipper
							...
							//We get here if we have flipped the utrip flag.
							&& (creep.memory.direction = Memory.rooms[room_name].path[pos.x][pos.y].upgrade[source]))
				...
				case "utransport":
				{
					...
					if (Memory.rooms[room_name].path[pos.x][pos.y].flipper
						...
						||  (Memory.rooms[room_name].path[pos.x][pos.y].flipper.ureturn && Memory.rooms[room_name].path[pos.x][pos.y].flipper.ureturn[source] && (!(creep.memory.return = false))
							//Everything seems to work except hitting the source on the return trip. So we need an exception here.
							&& Memory.rooms[room_name].path[pos.x][pos.y].upgrade && (creep.memory.direction = Memory.rooms[room_name].path[pos.x][pos.y].upgrade[source]))))
					{
						//We get here if we have flipped any flag.
					}
					break;
				...
				case "dbuilder":
				{
					if ((creep.memory.destination && (!creep.memory.return && Memory.rooms[room_name].path[pos.x][pos.y].patrol && (tempdir = Memory.rooms[room_name].path[pos.x][pos.y].patrol[creep.memory.need]))
