...
var roomPlanner =
{
	...
	setupDefense: function(room_name = false)
	{
		...
		let defense = {lastrp: null, lastmil: 1, walls: [], farwalls: [], ramparts: [], safe: Memory.rooms[room_name].safe};
	...
	removeOldPaths: function(room_name = false, level)
	{
		if (!room_name)
		{
			for (let room_name in Memory.rooms)
			{
				roomPlanner.removeOldPaths(room_name, level);
			}
			return true;
		}
		...
		switch(level)
		{
			case 1:	//After init is complete.
				for (let i = 0; i < Memory.rooms[room_name].sources.length; i++)
				{
					
				}
				break;
			case 2:	//After defense is complete.
